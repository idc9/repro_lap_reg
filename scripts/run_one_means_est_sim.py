...
# setup configuration
parser = argparse.ArgumentParser(description='Run a single simulation for means estimation with block diagonal shrinkage.')
parser = add_one_sim_config(parser)
parser = add_args_for_bayes(parser)

parser.add_argument('--noise_scale', default=1, type=float,
                    help='Scale of the noise.')

args = parser.parse_args()
maybe_submit_on_bayes(args)

# run the simulation!
run_sim_from_args(args, model_name='means_est', verbosity=np.inf)
